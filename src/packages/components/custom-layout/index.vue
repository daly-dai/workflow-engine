...
<template>
  <div class="wrap">
    ...
    >
      ...
      <div class="mb3">
        {{ `${parseInt(item.width * 62.5)}x${parseInt(item.height * 62.5)}` }}
      </div>
  ...
  watch: {
    ...
    cuurIndex: {
      handler(newValue, oldValue) {
        ...
        const temp = newValue === -1 ? undefined : this.mValue[newValue];
  ...
  methods: {
    ...
    clickWrap(e) {
      ...
      if (!this.edit) {
        ...
        const keys = cloneDeep(sortBy(this.editKeys));
        const start = this.splitKey(keys[0]);
        const end = this.splitKey(keys.pop());
    ...
    move(e) {
      ...
      const keys = [];
      const start = this.splitKey(this.startKey);
      const end = this.splitKey(e.target.dataset.key);
    ...
    antiCollision(start, end) {
      ...
      cloneDeep(this.mValue).forEach((item) => {
